...
var moveHistory = []; // Lưu lịch sử các bước đi
var capturedHistory = []; // Lưu các quân bị ăn
...
function killOppositePiece(obj, clickedPiece) {
    ...
    for (var i = 0; i < obj.obstaclesCanBeKilled.length; i++) {
        if ($(clickedPiece).attr('id') === obj.obstaclesCanBeKilled[i].attr('id')) {
            capturedHistory.push({ piece: $(clickedPiece).clone(), cell: $(clickedPiece).parent().attr("id") });
            $(clickedPiece).remove();
            // Lưu trạng thái trước khi di chuyển
    moveHistory.push({
        piece: obj,
...
function undoMove() {
    ...
    $("#" + piece.id).appendTo("#" + lastMove.from);
    piece.currentPosition = lastMove.from;
    piece.isFirstMove = false;

    selectedPiece = null;
    previousMovedPiece = null;

    // Khôi phục quân bị ăn nếu có
    if (capturedHistory.length > 0) {
        var lastCaptured = capturedHistory[capturedHistory.length - 1];
        if (lastCaptured.cell === lastMove.to) {
            $("#" + lastCaptured.cell).append(lastCaptured.piece);
            capturedHistory.pop();
        }
    }
}
